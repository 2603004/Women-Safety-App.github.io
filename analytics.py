from pathlib import Path
from tkinter import Tk, Canvas, Entry, Text, Button, PhotoImage,ttk
from matplotlib.figure import Figure
from matplotlib.backends.backend_tkagg import FigureCanvasTkAgg
import pandas as pd
OUTPUT_PATH = Path(__file__).parent
ASSETS_PATH = OUTPUT_PATH / Path(r"C:\Users\abhay\OneDrive\Desktop\Tkinter-Designer-master\build\assets\frame0")

revenue = { 
    "date": [
        "2022-08-19",
        "2022-08-20",
        "2022-08-23",
        "2022-08-24",
        "2022-08-25",
        "2022-08-26",
        "2022-08-27",
        "2022-08-30",
        "2022-08-31",
        "2022-09-01",
        "2022-09-02",
        "2022-09-03",
        "2022-09-07",
        "2022-09-08",
        "2022-09-09",
        "2022-09-10",
        "2022-09-13",
        "2022-09-14",
        "2022-09-15",
        "2022-09-16",
        "2022-09-17",
        "2022-09-20",
        "2022-09-21",
        "2022-09-22",
        "2022-09-23",
        "2022-09-24",
        "2022-09-27",
        "2022-09-28",
        "2022-09-29",
        "2022-09-30",
        "2022-10-01",
        "2022-10-04",
        "2022-10-05",
        "2022-10-06",
        "2022-10-07",
        "2022-10-08",
        "2022-10-11",
        "2022-10-12",
        "2022-10-13",
        "2022-10-14",
        "2022-10-15",
        "2022-10-18",
        "2022-10-19",
        "2022-10-20",
        "2022-10-21",
        "2022-10-22",
        "2022-10-25",
        "2022-10-26",
        "2022-10-27",
        "2022-10-28",
        "2022-10-29",
        "2022-11-01",
        "2022-11-02",
        "2022-11-03",
        "2022-11-04",
        "2022-11-05",
        "2022-11-08",
        "2022-11-09",
        "2022-11-10",
        "2022-11-11",
        "2022-11-12",
        "2022-11-15",
        "2022-11-16",
        "2022-11-17",
        "2022-11-18",
        "2022-11-19",
        "2022-11-22",
        "2022-11-23",
        "2022-11-24",
        "2022-11-26",
        "2022-11-29",
        "2022-11-30",
        "2022-12-01",
        "2022-12-02",
        "2022-12-03",
        "2022-12-06",
        "2022-12-07",
        "2022-12-08",
        "2022-12-09",
        "2022-12-10",
        "2022-12-13",
        "2022-12-14",
        "2022-12-15",
        "2022-12-16",
        "2022-12-17",
        "2022-12-20",
        "2022-12-21",
        "2022-12-22",
        "2022-12-23",
        "2022-12-27",
        "2022-12-28",
        "2022-12-29",
        "2022-12-30",
        "2022-12-31",
        "2023-01-03",
        "2023-01-04",
        "2023-01-05",
        "2023-01-06",
        "2023-01-07",
        "2023-01-10",
        "2023-01-11",
        "2023-01-12",
        "2023-01-13",
        "2023-01-14",
        "2023-01-18",
        "2023-01-19",
        "2023-01-20",
        "2023-01-21",
        "2023-01-24",
        "2023-01-25",
        "2023-01-26",
        "2023-01-27",
        "2023-01-28",
        "2023-01-31",
        "2023-02-01",
        "2023-02-02",
        "2023-02-03",
        "2023-02-04",
        "2023-02-07",
        "2023-02-08",
        "2023-02-09",
        "2023-02-10",
        "2023-02-11",
        "2023-02-14",
        "2023-02-15",
        "2023-02-16",
        "2023-02-17",
        "2023-02-18",
        "2023-02-22",
        "2023-02-23",
        "2023-02-24",
        "2023-02-25",
        "2023-02-28",
        "2023-03-01",
        "2023-03-02",
        "2023-03-03",
        "2023-03-04",
        "2023-03-07",
        "2023-03-08",
        "2023-03-09",
        "2023-03-10",
        "2023-03-11",
        "2023-03-14",
        "2023-03-15",
        "2023-03-16",
        "2023-03-17",
        "2023-03-18",
        "2023-03-21",
        "2023-03-22",
        "2023-03-23",
        "2023-03-24",
        "2023-03-28",
        "2023-03-29",
        "2023-03-30",
        "2023-03-31",
        "2023-04-01",
        "2023-04-04",
        "2023-04-05",
        "2023-04-06",
        "2023-04-07",
        "2023-04-08",
        "2023-04-11",
        "2023-04-12",
        "2023-04-13",
        "2023-04-14",
        "2023-04-15",
        "2023-04-18",
        "2023-04-19",
        "2023-04-20",
        "2023-04-21",
        "2023-04-22",
        "2023-04-25",
        "2023-04-26",
        "2023-04-27",
        "2023-04-28",
        "2023-04-29",
        "2023-05-02",
        "2023-05-03",
        "2023-05-04",
        "2023-05-05",
        "2023-05-06",
        "2023-05-09",
        "2023-05-10",
        "2023-05-11",
        "2023-05-12",
        "2023-05-13",
        "2023-05-16",
        "2023-05-17",
        "2023-05-18",
        "2023-05-19",
        "2023-05-20",
        "2023-05-23",
        "2023-05-24",
        "2023-05-25",
        "2023-05-26",
        "2023-05-27",
        "2023-05-31",
        "2023-06-01",
        "2023-06-02",
        "2023-06-03",
        "2023-06-06",
        "2023-06-07",
        "2023-06-08",
        "2023-06-09",
        "2023-06-10",
        "2023-06-13",
        "2023-06-14",
        "2023-06-15",
        "2023-06-16",
        "2023-06-17",
        "2023-06-20",
        "2023-06-21",
        "2023-06-22",
        "2023-06-23",
        "2023-06-24",
        "2023-06-27",
        "2023-06-28",
        "2023-06-29",
        "2023-06-30",
        "2023-07-01",
        "2023-07-05",
        "2023-07-06",
        "2023-07-07",
        "2023-07-08",
        "2023-07-11",
        "2023-07-12",
        "2023-07-13",
        "2023-07-14",
        "2023-07-15",
        "2023-07-18",
        "2023-07-19",
        "2023-07-20",
        "2023-07-21",
        "2023-07-22",
        "2023-07-25",
        "2023-07-26",
        "2023-07-27",
        "2023-07-28",
        "2023-07-29",
        "2023-08-01",
        "2023-08-02",
        "2023-08-03",
        "2023-08-04",
        "2023-08-05",
        "2023-08-08",
        "2023-08-09",
        "2023-08-10",
        "2023-08-11",
        "2023-08-12",
        "2023-08-15",
        "2023-08-16",
        "2023-08-17",
        "2023-08-18",
        "2023-08-19",
        "2023-08-22",
        "2023-08-23",
        "2023-08-24",
        "2023-08-25",
        "2023-08-26",
        "2023-08-29",
        "2023-08-30",
        "2023-08-31",
    ],
    "amount": [
        105.73,
        107.15,
        107.04,
        106.23,
        105.52,
        104.61,
        102.7,
        101.54,
        101.04,
        100.84,
        101.35,
        101.55,
        102.88,
        104.36,
        106.66,
        109.08,
        111.24,
        113.74,
        115.7,
        117.16,
        118.27,
        119.1,
        119.22,
        119.32,
        121.44,
        124.01,
        126.45,
        130.03,
        132.08,
        133.9,
        135.77,
        137.34,
        138.01,
        137.23,
        137.31,
        137.82,
        138.89,
        141.1,
        144.04,
        145.8,
        145.43,
        146.74,
        152.56,
        162.43,
        172.75,
        181.9,
        187.12,
        187.93,
        191.48,
        193.71,
        193.3,
        191.82,
        185.15,
        179.57,
        173.82,
        169.62,
        173.03,
        175.4,
        179.44,
        180.61,
        177.98,
        174.36,
        170.49,
        168.64,
        167.39,
        169.2,
        171.69,
        175.68,
        179.3,
        180.6,
        180.6,
        180.44,
        179.01,
        176.88,
        174.52,
        172.78,
        171.62,
        171.38,
        173.56,
        175.14,
        176.35,
        178.76,
        180.34,
        181.33,
        183.79,
        185.74,
        187.46,
        189.72,
        191.37,
        193.79,
        194.01,
        196.5,
        196.9,
        195.88,
        194.82,
        192.6,
        192.74,
        192.75,
        194.46,
        194.83,
        196.06,
        198.64,
        199.12,
        198.77,
        195.85,
        190.06,
        185.01,
        183.84,
        183.79,
        186.2,
        190.82,
        191.48,
        195.96,
        201.08,
        203.27,
        204.3,
        202.47,
        197.65,
        193.56,
        191.4,
        189.99,
        190.9,
        193.51,
        196.13,
        197.37,
        196.41,
        195.29,
        193.04,
        190.02,
        189.18,
        187.2,
        186.28,
        186.56,
        186.04,
        186.73,
        186.73,
        185.32,
        183.84,
        181.08,
        178.53,
        177.84,
        176.75,
        177.12,
        178.38,
        178.95,
        179.7,
        179.62,
        179.43,
        179.56,
        179.8,
        180.17,
        180.49,
        180.14,
        181.57,
        183.6,
        185.78,
        189.21,
        190.9,
        192.06,
        193.25,
        193.04,
        192.89,
        190.83,
        189.09,
        188.7,
        190.37,
        195.17,
        202.38,
        210.42,
        215.58,
        219.47,
        220.38,
        219.5,
        220.38,
        221.98,
        224.24,
        225.99,
        227.42,
        227.38,
        227.2,
        228.28,
        228.46,
        229.26,
        230.08,
        230.54,
        233.14,
        235.63,
        238.27,
        243.85,
        248.06,
        253.47,
        257.86,
        260.5,
        265.82,
        272.62,
        279.79,
        283.36,
        286.77,
        288.05,
        285.97,
        287.48,
        285.37,
        282.78,
        282.48,
        279.6,
        278.34,
        277.72,
        279.81,
        283.07,
        286.03,
        288.39,
        291.02,
        295.09,
        298.26,
        299.02,
        298.24,
        295.03,
        293.46,
        293.16,
        293.5,
        294.75,
        294.16,
        294.23,
        295.06,
        296.22,
        298.18,
        300.12,
        302.88,
        305.66,
        308.85,
        309.56,
        306.05,
        302.07,
        297.82,
        295.59,
        293.57,
        292.45,
        293.02,
        293.96,
        296.46,
        296.64,
        294.66,
        293.23,
        290.21,
        288.14,
        287.76,
        285.86,
        285.86,
        286.12,
        283.68,
        282.68,
        279.77,
        278.4,
        279.04,
        279.69,
        282.08,
        284.3,
        285.47,
        286.32,
        286.99,
        286.99,
        286.95,
    ]
}

# Sales data
sales = {
    "revenue": [472 ,356,278, 412, 550, 672],
    "products": ["Women 1", "Women 2", "Women 3", "Women 4", "Women 5", "Women 6"],
    "angles": [0, 1.04719, 2.094395, 3.141592, 4.18879, 5.23598],
    "rotation": [-90, -30, 30, 90, 330, 390, 450],
    "colors": ["#439A97", "#56a8a5", "#6cbdba", "#12b2ed", "#008fc4", "#0a7ca6"]
}

# Table data

table_columns = ["ID", "Customer", "Product"]
table_data = [    
    ["1", "Jack", "Women 1"],
    ["2", "Abby", "Women 1"],
    ["3", "Abby", "Women 1"],
    ["4", "Max", "Women 2"],
    ["5", "Jack", "Women 2"],
    ["6", "Abby", "Women 2"],
    ["7", "Max", "Women 3"],
    ["8", "Jack", "Women 3"],
    ["9", "Abby", "Women 3"],
    ["10", "Abby", "Women 3"],
    ["11", "Jack", "Women 4"],
    ["12", "Abby", "Women 4"],
    ["13", "Abby", "Women 4"],
    ["14", "Max", "Women 5"],
    ["15", "Jack", "Women 5"],
    ["16", "Abby", "Women 5"],
    ["17", "Max", "Women 6"],
    ["18", "Jack", "Women 6"],
    ["19", "Abby", "Women 6"],
    ["20", "Abby", "Women 6"],
]

def relative_to_assets(path: str) -> Path:
    return ASSETS_PATH / Path(path)


window = Tk()

window.geometry("700x408")
window.configure(bg = "#19264C")


canvas = Canvas(
    window,
    bg = "#19264C",
    height = 408,
    width = 700,
    bd = 0,
    highlightthickness = 0,
    relief = "ridge"
)

canvas.place(x = 0, y = 0)
image_image_1 = PhotoImage(
    file=relative_to_assets("image.png"))
image_1 = canvas.create_image(
    350.0,
    24.0,
    image=image_image_1
)

canvas.create_text(
    56.0,
    13.0,
    anchor="nw",
    text="SafeAnalytics",
    fill="#060605",
    font=("Inter Bold", 20 * -1)
)

image_image_2 = PhotoImage(
    file=relative_to_assets("image_2.png"))
image_2 = canvas.create_image(
    31.0,
    23.0,

    image=image_image_2
)

image_image_3 = PhotoImage(
    file=relative_to_assets("image_5.png"))
image_3 = canvas.create_image(
    120.0,
    227.0,
    image=image_image_3
)

image_image_4 = PhotoImage(
    file=relative_to_assets("image_5.png"))
image_4 = canvas.create_image(
    574.0,
    227.0,
    image=image_image_4
)

image_image_5 = PhotoImage(
    file=relative_to_assets("image_5.png"))
image_5 = canvas.create_image(
    347.0,
    227.0,
    image=image_image_5
)

revenue_data = pd.DataFrame(revenue)
revenue_data["date"] = pd.to_datetime(revenue_data["date"])
#fig_1
fig_1=Figure(figsize=(2.05, 2.25), facecolor="#19264C")
ax_1= fig_1.add_subplot()
ax_1.set_facecolor("#917FB3")
ax_1.fill_between(x=revenue_data["date"], y1=revenue_data["amount"], alpha=0.7)
ax_1.tick_params(labelsize=7, colors="white")
fig_1.autofmt_xdate()
ax_1.plot(revenue_data["date"], revenue_data["amount"], color="deepskyblue")
ax_1.grid(visible=True)

canvas = FigureCanvasTkAgg(figure=fig_1, master=window)
canvas.draw()
canvas.get_tk_widget().place(x=17,y=120)
#fig_3
fig_2=Figure(figsize=(2.05, 2.25), facecolor="#19264C")
ax_2 = fig_2.add_subplot(projection="polar")

ax_2.set_facecolor("#917FB3")

ax_2.bar(x=sales["angles"], height=sales["revenue"], color=sales["colors"])

ax_2.set_frame_on(False)

ax_2.set_xticks([])
ax_2.tick_params(labelsize=2, colors="white")

ax_2.grid(alpha=0.7)

for angle, label, rotation in zip(sales["angles"], sales["products"], sales["rotation"]):
 ax_2.text(x=angle, y=max(sales ["revenue"]) + 30, s=label, rotation=rotation, ha="center", va="center", color="white",fontsize=8)

canvas=FigureCanvasTkAgg(figure=fig_2, master=window)
canvas.draw()
canvas.get_tk_widget().place(x=472,y=120)

#fig_2
table = ttk.Treeview(master=window, columns=table_columns, show="headings")

for column in table_columns:
    table.heading(column=column, text=column) 
    table.column(column=column, width=60)

for row_data in table_data:
    table.insert(parent="", index="end", values=row_data)

style = ttk.Style()
style.theme_use("default")
style.configure("Treeview", background="#19264C", fieldbackground="#19264C", foreground="white")
style.configure("Treeview.Heading", background="#19264C", fieldbackground="#19264C", foreground="white") 
style.map("Treeview", background=[("selected", "#19264C")])

table.place(x=257,y=95,height=260)
                             
window.resizable(False, False)
window.mainloop()
